...
def create(request):
    if request.method == "POST":
        result = Users.usermanager.registerValid(request.POST['name'], request.POST['alias'],request.POST['email'], request.POST['password'], request.POST['conf_pw'])
        if result[0]:
            request.session['alias'] = result[1].alias
            request.session['logged_id'] = results[1].id
            try:
                request.session.pop('errors')
            except:
                pass
